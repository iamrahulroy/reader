...
describe Feed do
  describe "#merge" do
    ...
    let(:subscription_a)         { Subscription.create! user: user_a, source_id: feed_a.id, source_type: 'Feed', name: "User Subscription", group: group_a }
    let(:subscription_b)           { Subscription.create! user: user_b, source_id: feed_b.id, source_type: 'Feed', name: "User Subscription", group: group_b }
    ...
    let(:entry_a)  { Entry.create! guid: "123", url: "http://www.example.com/", source_id: feed_a.id, source_type: 'Feed', published_at: Date.current }
    let(:entry_b)  { Entry.create! guid: "123", url: "http://www.example.com/", source_id: feed_b.id, source_type: 'Feed', published_at: Date.current }
    ...
    let(:entry_c)  { Entry.create! guid: "124", url: "http://www.example.com/", source_id: feed_a.id, source_type: 'Feed', published_at: Date.current }
    ...
    it "re-parents a feeds entries" do
      Entry.where(source_id: feed_b.id, source_type: 'Feed').count.should eq 0
    end

    it "re-parents a feeds subscriptions" do
      Subscription.where(source_id: feed_b.id, source_type: 'Feed').count.should eq 0
    end

    it "cleans up the old entry_guids" do
      EntryGuid.where(source_id: feed_b.id, source_type: 'Feed').count.should eq 0
    end
    ...
    it "deletes duplicate entries" do
      Entry.where(source_id: feed_b.id, source_type: 'Feed').count.should eq 0
      Entry.where(source_id: feed_a.id, source_type: 'Feed').count.should eq 2
    end
