...
#    App.updateKeyBindingsForList()
  ...
  App.autoPlay = ->
    $(document).off('keypress.shift_1', App.autoPlay)
    $(document).on('keypress.shift_1', App.stopAutoPlay)
    if App.intervalId?
      window.clearInterval(App.intervalId)
    App.intervalId = window.setInterval(App.nextItem, 3000)

  App.stopAutoPlay = ->
    if App.intervalId?
      window.clearInterval(App.intervalId)
    $(document).on('keypress.shift_1', App.autoPlay)
