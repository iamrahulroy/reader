$(document).on "change", "#share-to-twitter", (evt) ->
  App.user.set("share_to_twitter", evt.target.checked)
  App.user.save()

$(document).on "change", "#share-to-facebook", (evt) ->
  App.user.set("share_to_facebook", evt.target.checked)
  App.user.save()
...
App.loadOptions = ->
  $('#tab-options').load('/settings/options')
