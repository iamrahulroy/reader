class App.Item extends Backbone.Model
  ...
  postToTwitter: =>
    unless @get("sent_to_twitter") == true
      $.post(this.twitter_url()).success(@postToTwitterSuccess)
  postToTwitterSuccess: (data, status, xhr) =>
    @.trigger('item:tweeted')

  postToFacebook: =>
    unless @get("sent_to_facebook") == true
      $.post(this.facebook_url()).success(@postToFacebookSuccess)
  postToFacebookSuccess: (data, status, xhr) =>
    @.trigger('item:facebooked')
