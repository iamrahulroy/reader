class GroupsController < ApplicationController
  ...
  def index
    ...
    render :json => @groups, :each_serializer => GroupSerializer, :root => false
  end
  ...
  def create
    ...
    if @group
      ...
      if @group.save
        render :json => @group, :status => :created, :root => false
      else
        render :json => @group.errors, :status => :unprocessable_entity, :root => false
      end
    end
  end
  ...
  def update
    ...
    @group.save
    render :json => @group, :root => false #, :serializer => GroupSerializer
  end
